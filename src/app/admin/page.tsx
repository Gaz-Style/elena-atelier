...
export default function AdminDashboard() {
  ...
  const operationalModules = [
    ...
    { title: 'Catálogo de Servicios', desc: 'Gestión de productos y oferta comercial.', href: '/admin/catalog', icon: Package, size: 'col-span-1', accentColor: 'group-hover:text-emerald-600 group-hover:bg-emerald-50 group-hover:border-emerald-100/80' },
  ];

  const financialModules = [
    { title: 'Planilla de Ventas', desc: 'Libro Mayor de ingresos y caja histórica.', href: '/admin/sales', icon: DollarSign, size: 'col-span-1', accentColor: 'group-hover:text-sky-600 group-hover:bg-sky-50 group-hover:border-sky-100/80' },
    { title: 'Presupuestos & Cotizaciones', desc: 'Seguimiento de propuestas enviadas.', href: '/admin/quotes', icon: FileText, size: 'col-span-1', accentColor: 'group-hover:text-sky-600 group-hover:bg-sky-50 group-hover:border-sky-100/80' },
    { title: 'Finanzas & Contabilidad', desc: 'Impuestos SII, conciliación y balances P&L.', href: '/admin/finance', icon: DollarSign, size: 'col-span-2', accentColor: 'group-hover:text-sky-600 group-hover:bg-sky-50 group-hover:border-sky-100/80' },
    { title: 'Inventario de Taller', desc: 'Metraje de telas, hilos y suministros.', href: '/admin/inventory', icon: Package, size: 'col-span-1', accentColor: 'group-hover:text-sky-600 group-hover:bg-sky-50 group-hover:border-sky-100/80' },
    { title: 'Recursos Humanos', desc: 'Vacaciones, personal y nóminas (Buk).', href: '/admin/hr', icon: Users, size: 'col-span-1', accentColor: 'group-hover:text-sky-600 group-hover:bg-sky-50 group-hover:border-sky-100/80' },
  ];
    ...
    <div className="min-h-screen bg-zinc-50/30 font-sans text-zinc-800 selection:bg-zinc-200/60">
      <main className="max-w-7xl mx-auto px-6 md:px-8 py-8 space-y-10">
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <PosMetricsCard data={detailedData?.cashRegister} isLoading={loading} />
                ...
                <div>
                  ...
                  {loading ? (
                    <div className="space-y-3">
                      {[1, 2, 3].map(i => (
                        <div key={i} className="h-8 bg-zinc-50 rounded-lg animate-pulse" />
                      ))}
                    </div>
