...
const newTemplatesStr = `const DEFAULT_HC_TEMPLATES = [
    {
        id: 'vestido_gala',
        name: 'Vestido Gala',
...
c = c.replace(/const DEFAULT_HC_TEMPLATES = \[\s*\{[\s\S]*?\];/m, newTemplatesStr);

c = c.replace(/import \{ sendBudgetEmailAction.*?\}/s, "import { sendBudgetEmailAction, sendOrderConfirmationEmailAction, createPOSOrdersAction, checkOrderStatusAction, getDailyWorkloadAction, getEstimatedDatesAction, getOperatorsAction, getAtelierConfigAction, saveBudgetAction, wakeUpMercadoPagoTerminalAction, requestDiscountAuthorizationAction, getOperatorsDailyLoadAction, analyzeDesignWithGeminiAction, updateAtelierTemplatesAction } from './actions';\nimport TemplateManager from './TemplateManager';");
...
c = c.replace(/<div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-8">/, tabsStr);
...
c = c.replace(/<\/div>\s*<\/div>\s*<\/div>\s*<div className="p-4 md:p-6 border-t border-gray-100 flex justify-end gap-3 bg-gray-50 shrink-0">/, tabsEndStr);

// FIX: Ensure || is only replaced with ?? inside value={...}
